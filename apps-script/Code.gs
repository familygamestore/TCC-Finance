  ...
  try {
    ...
    switch (action) {
      ...
      case 'event_detail':
        ...
        break;
      case 'categories':
        result = getSheetAsObjects(SHEETS.CATEGORIES).filter(r => String(r.status || '').toLowerCase() !== 'inactive' && String(r.status || '').toLowerCase() !== 'nonaktif');
        break;
      case 'payment_methods':
        result = getSheetAsObjects(SHEETS.PAYMENT_METHODS).filter(r => String(r.status || '').toLowerCase() !== 'inactive' && String(r.status || '').toLowerCase() !== 'nonaktif');
        break;
      case 'report':
...
function createTransaction(sheetName, body, prefix) {
  ...
  validateCategory(body.kategori, expectedType);
...
function uploadBukti(body) {
  ...
  if (estimatedBytes > MAX_UPLOAD_BYTES) throw new Error('Ukuran bukti maksimal 8 MB.');
...
function validateCategory(categoryName, type) {
  if (!categoryName) throw new Error('Kategori wajib dipilih.');
  const categories = getSheetAsObjects(SHEETS.CATEGORIES);
  const found = categories.find(r => String(r.nama_kategori || '').trim() === String(categoryName).trim() && String(r.tipe || '').toLowerCase() === type && !['inactive','nonaktif'].includes(String(r.status || '').toLowerCase()));
  if (!found) throw new Error('Kategori tidak valid atau tidak aktif untuk tipe transaksi ini.');
}
...
function getTransactions(params) {
  ...
  if (params.tanggal_dari && params.tanggal_sampai) {
    const from = normalizeDateFilter(params.tanggal_dari);
    const to = normalizeDateFilter(params.tanggal_sampai);
    all = all.filter(r => { const d = normalizeDateFilter(r.tanggal); return d >= from && d <= to; });
  }

  all.sort((a, b) => sheetDateTimeKey(b.created_at) - sheetDateTimeKey(a.created_at));
  return all;
}
...
function getReport(params) {
  ...
  const filterByDate = (rows) => {
    ...
    const from = normalizeDateFilter(params.tanggal_dari);
    const to = normalizeDateFilter(params.tanggal_sampai);
    return rows.filter(r => { const d = normalizeDateFilter(r.tanggal); return d >= from && d <= to; });
  };
...
function getSheetAsObjects(sheetName) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    headers.forEach((h, idx) => {
      obj[h] = normalizeSheetValue(h, data[i][idx]);
    });
    rows.push(obj);
  }
  return rows;
...
function normalizeSheetValue(header, value) {
  ...
  if (key === 'tanggal' || key === 'tanggal_mulai' || key === 'tanggal_selesai') {
    return formatDate(value);
  }
  if (key === 'jam') {
    return formatTime(value);
  }
  if (key === 'created_at' || key === 'waktu') {
...
function normalizeDateFilter(value) {
  ...
  return isNaN(d.getTime()) ? raw : Utilities.formatDate(d, TIMEZONE, 'yyyyMMdd');
}

function sheetDateTimeKey(value) {
  const raw = String(value || '').trim();
  const m = raw.match(/^(\d{2})\/(\d{2})\/(\d{4})(?:\s+(\d{2}):(\d{2})(?::(\d{2}))?)?$/);
  if (!m) { const d = new Date(raw); return isNaN(d.getTime()) ? 0 : d.getTime(); }
