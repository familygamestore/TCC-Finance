...
export default function EventForm({form,setForm,onSubmit,submitting,brands,mode='create'}:{form:EventFormData;setForm:(f:EventFormData)=>void;onSubmit:(f:EventFormData)=>void;submitting:boolean;brands:Brand[];mode?:'create'|'request'}){
 ...
 return <form className="panel event-builder" onSubmit={submit}>
  <div className="form-title"><strong>{mode==='create'?'Event Builder':'Event Request'}</strong><span>Buat event apa pun: 4–256 tim, game bebas, sistem turnamen bebas, biaya & budget cepat atau manual.</span></div>
  ...
  <button className="btn" type="submit" disabled={submitting}>{submitting?'Mengirim…':mode==='create'?'＋ Simpan Event':'＋ Kirim Pengajuan'}</button>
 </form>
}
